...
async def add_new_admins(client: Client, query: CallbackQuery):
    ...
    if query.from_user.id != client.owner:
        return await query.answer('This can only be used by owner.', show_alert=True)
    ...
    try:
        for identifier in ids:
            if int(identifier) == client.owner:
                await client.send_message(query.from_user.id, "The owner cannot be removed from the admin list.")
                continue
